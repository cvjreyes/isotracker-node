...
const transaction = async (req, res) => {
  ...
  sql.query('SELECT * FROM dpipes_view WHERE isoid COLLATE utf8mb4_unicode_ci = ?', [fileName.split('.').slice(0, -1)], (err, results) => {
    if (!results[0] && process.env.NODE_PROGRESS == "1") {
      ...
      sql.query('SELECT * FROM users WHERE email = ?', [req.body.user], (err, results) => { //Cogemos el user que hace la transaccion
        if (!results[0]) {
      ...
      sql.query("SELECT created_at FROM hisoctrls WHERE filename = ?", req.body.fileName, (err, results) => {
        if (!results[0]) {
          ...
          sql.query("SELECT * FROM misoctrls WHERE filename = ?", req.body.fileName, (err, results) => { //Cogemos la iso sobre la que se hace la transaccion
            if (!results[0]) {
              res.status(401).send("File not found");
            } else {
              let revisionCompleted = false
              //Comporbamos si tiene los campos de revision completos en caso de pasar de issuer a LOS
              ...
              if (results[0].returned == 1 && req.body.to == "Supports" && req.body.role == "StressLead") { //Si anteriormente habia sido retornada de soportes a stress
                sql.query('SELECT user, role FROM hisoctrls WHERE filename = ? AND `to` = ? AND role = ? ORDER BY id DESC LIMIT 1', [req.body.fileName, "Claimed", "SupportsLead"], (err, results) => { //Obtenemos el lider de soporte que la tuvo
                  if (!results[0]) {
                    ...
                    sql.query('SELECT * from misoctrls WHERE filename = ?', [fileName], (err, results) => {
                      if (!results[0]) {
                        ...
                        //Movemos los archivos fisicos en el storage
                          ...
                          sql.query("INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, `from`, `to`, comments, user, role) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                            [fileName, results[0].revision, results[0].spo, results[0].sit, results[0].deleted, results[0].onhold, from_text, destiny, "Unclaimed by leader", dest_user, dest_role], (err, results) => {
                              if (err) {
                                ...
                                if (process.env.NODE_PROGRESS == "1") { //Si el proyecto tiene progreso cogemos el progreso de la iso
                                  let type = ""
                                        ...
                                        if (!results[0]) {
                                          ...
                                          sql.query("SELECT progress, max_tray FROM misoctrls WHERE filename = ?", [fileName], (err, results) => {
                                            if (!results[0]) {
                                              ...
                                              //Actualizamos misoctrls con la nueva transaccion
                                ...
                                } else { //Si no tiene progreso simplemente actualizamos la iso
              ...
              } else { //Si no viene de soportes a stress es una transaccion normal
                const from = results[0].to
                let created_at = results[0].created_at
                if (!fs.existsSync('./app/storage/isoctrl/' + from + "/attach/" + req.body.fileName.split('.').slice(0, -1).join('.') + '-CL.pdf') && req.body.to == "LDE/Isocontrol") {//No puede pasar a LOS sin tener clean
                  res.status(401).send({ error: "error" })
                } else if (!revisionCompleted && process.env.NODE_PROGRESS == "1" && process.env.NODE_ISSUER == "1" && req.body.to == "LDE/Isocontrol") { //tampoco sin los campos de revision rellenos
                  res.status(401).send({ error: "rev" })
                } else { //Si no es el caso y la transaccion es correcta segun el flujo de isotracker la guardamos en el historial
                  sql.query("INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                    [req.body.fileName, results[0].revision, results[0].spo, results[0].sit, req.body.deleted, req.body.onhold, from, req.body.to, req.body.comment, req.body.role, username], (err, results) => {
                      if (err) {
                        ...
                        if (req.body.to == "Recycle bin") { //Si la iso se quiere eiminar
                          let fromR = from
                          if (from == "LDE/Isocontrol") {
                            fromR = "lde"
                          }

                          //Movemos los archivos fisicos del storage
                        ...
                        } else if (req.body.to == "On hold") {//Si la iso se envia a hold
                        ...
                        if (process.env.NODE_PROGRESS == "1" && req.body.to !== "Recycle bin" && req.body.to !== "On hold") {
                          ...
                          //Como tenemos progreso se coge el progreso de la iso y se actualiza en funcion de la nueva bandeja
                        ...
                        } else { //Si no tenemos progreso simplemente acutalizamos misoctrls
...
const returnLead = async (req, res) => {//Para retornar a un lider el proceso es igual al anterior pero solo para este caso (esto podria ser otro if dentro del transaction)
  const fileName = req.body.fileName
  sql.query('SELECT * FROM dpipes_view WHERE isoid COLLATE utf8mb4_unicode_ci = ?', [fileName.split('.').slice(0, -1)], (err, results) => {
    if (!results[0] && process.env.NODE_PROGRESS == "1") {
      sql.query('UPDATE misoctrls SET blocked = 1 WHERE filename = ?', [fileName], (err, results) => {
...
const returnLeadStress = async (req, res) => { //Para retornar a un lider de stress el proceso es igual al anterior pero solo para este caso (esto podria ser otro if dentro del transaction)
...
const returnIso = async (req, res) => { //reutrn normal de una iso a una bandeja anterior
  ...
  sql.query('SELECT * FROM dpipes_view WHERE isoid COLLATE utf8mb4_unicode_ci = ?', [fileName.split('.').slice(0, -1)], (err, results) => {
    if (!results[0] && process.env.NODE_PROGRESS == "1") {
      ...
      sql.query('SELECT * FROM users WHERE email = ?', [user], (err, results) => { //Cogemos el email
        if (!results[0]) {
          res.status(401).send("Username or password incorrect");
        } else {
          username = results[0].name
        }
        //Buscamos en el historial el ultimo usuario que tenia la iso en la bandeja a la que queremos retornar la iso
        sql.query('SELECT user, role FROM hisoctrls WHERE filename = ? AND `from` = ? AND role = ? ORDER BY id DESC LIMIT 1', [fileName, destiny, dest_role], (err, results) => {
          if (!results[0]) {
              ...
              if (!results[0]) {
                ...
                sql.query("SELECT * FROM misoctrls WHERE filename = ?", fileName, (err, results) => { //Cogemos la iso
                  if (!results[0]) {
                    ...
                    const from = results[0].to
                    let created_at = results[0].created_at
                    //Si lo movemos a LOS comprobamos que tenga clean
                    if (!fs.existsSync('./app/storage/isoctrl/' + from + "/attach/" + fileName.split('.').slice(0, -1).join('.') + '-CL.pdf') && destiny == "LDE/Isocontrol") {
                      res.status(401)
                    } else {
                      //Creamos la transaccion en el historial
                      sql.query("INSERT INTO hisoctrls (filename, revision, spo, sit, deleted, onhold, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?)",
                        [fileName, results[0].revision, results[0].spo, results[0].sit, 0, 0, from, destiny, comments, role, username], (err, results) => {
                          if (err) {
                            console.log("error: ", err);
                          } else {
                            //Movemos los archivos en el storage
                            let masterName, origin_path, destiny_path, origin_attach_path, destiny_attach_path, origin_cl_path, destiny_cl_path, origin_proc_path, destiny_proc_path, origin_inst_path, destiny_inst_path = ""
                            masterName = fileName.split('.').slice(0, -1)
                            ...
                            if (process.env.NODE_PROGRESS == "1") { //Si el proyecto tiene progreso lo sacamos de misoctrls
                              let type = ""
                              if (process.env.NODE_IFC == "0") {
                                type = "value_ifd"
                              } else {
                            ...
                            } else { //Si no tiene progreso simplemente actualizamos la iso en misoctrls
          ...
          } else { //Si ningun usuario tuvo la iso en la bandeja a la que se devuelve es el mismo proceso pero no se le asigna a ningun usuario
            const dest_user = results[0].user
            const dest_role = results[0].role
            sql.query('SELECT * from misoctrls WHERE filename = ?', [fileName], (err, results) => { 
              if (!results[0]) {
...
const returnToLOS = async (req, res) => {
  ...
  const role = req.body.role
  sql.query("SELECT name FROM users WHERE email = ?", email, (err, results) => { //Cogemos el email del usuario
    const username = results[0].name
    //Cogemos los datos de la iso emitida
    sql.query('SELECT transmittal, issued_date, isoid, revision, spo, sit, spoclaimed, comments FROM misoctrls WHERE filename = ?', [fileName], (err, results) => {
      if (!results[0]) {
        ...
        //Guardamos en el historial la transaccion
        sql.query("INSERT INTO hisoctrls (filename, revision, spo, sit, issued, transmittal, issued_date, deleted, onhold, `from`, `to`, comments, role, user) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
          [fileName, last.revision - 1, last.spo, last.sit, 0, null, null, last.deleted, last.onhold, "Issued", "LDE/IsoControl", "Returned to LOS", role, username], (err, results) => {
            if (err) {
              ...
              //Actualizamos la iso en el misoctrls
